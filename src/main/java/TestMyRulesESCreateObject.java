                        ...
                        for (RuleForWeka item : knowledgeBaseWeka.getRuleForWekaArrayList()) {
                                ...
                                for (ConditionForWeka it : item.getList()) {
                                        if (it.getTypeClass().equals("java.lang.String")) {
                                                System.out.println(it.getField() + " _ " + it.getValue().toString() + " _ " + it.getTypeClass());
                                                ft.set(fcObject, it.getField(), it.getValue());
                                        } else {
                                                System.out.println(it.getField() + " _ " + it.getValue().toString() + " _ " + it.getTypeClass());
                                                if (it.getValue().toString() == "0") {
                                                        //   ft.set(fcObject, it.getField(), null);
                                                        System.out.println(fcObject + " " + it.getField() + " не может быть 0 " + it.getValue());
                                                } else {
                                                        ft.set(fcObject, it.getField(), Long.valueOf(it.getValue().toString()));
                                                }
