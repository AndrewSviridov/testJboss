 ...
 */
public class MyPruneableDecList extends MyClassifierDecList {
     ...
     */
    public MyPruneableDecList(ModelSelection toSelectLocModel, int minNum) {
     ...
     * @throws Exception if something goes wrong
     */
    protected MyPruneableDecList getNewDecList(Instances train, Instances test,
                                               boolean leaf) throws Exception {

        MyPruneableDecList newDecList = new MyPruneableDecList(m_toSelectModel,
                m_minNumObj);
        ...
        if (m_isLeaf) {
            ...
            for (int i = 0; i < m_sons.length; i++) {
                if (Utils.eq(son(i).localModel().distribution().total(), 0)) {
                    ...
                    error += ((MyPruneableDecList) son(i)).errorsForTree();
                }
            }
